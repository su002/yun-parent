package com.yun.ssyx.user.controller;

import com.alibaba.fastjson.JSONObject;
import com.yun.ssyx.common.auth.AuthContextHolder;
import com.yun.ssyx.common.constant.RedisConst;
import com.yun.ssyx.common.exception.SsyxException;
import com.yun.ssyx.common.result.Result;
import com.yun.ssyx.common.result.ResultCodeEnum;
import com.yun.ssyx.common.utils.JwtHelper;
import com.yun.ssyx.enums.UserType;
import com.yun.ssyx.enums.user.User;
import com.yun.ssyx.user.service.UserService;
import com.yun.ssyx.user.utils.ConstantPropertiesUtil;
import com.yun.ssyx.user.utils.HttpClientUtils;
...
//        openid = "odo3j4q2KskkbbW-krfE-cAxUnzU1";
    ...
    //微信授权登录
    public Result loginWx1(@PathVariable String code) {
        // 1 得到微信返回临时code

        // 2 根据code 和小程序的id , 密钥 请求微信服务接口

        // 2.1 使用HttpClient 工具的方法发起请求
        String wxOpenAppId = ConstantPropertiesUtil.WX_OPEN_APP_ID;
        String wxOpenAppSecret = ConstantPropertiesUtil.WX_OPEN_APP_SECRET;
        //拼接请求参数
        StringBuffer url = new StringBuffer()
        ...
        String tokenUrl = String.format(url.toString(),
                ...
                wxOpenAppSecret,
                code);
        System.out.println(url.toString()+ "==" + tokenUrl);
        ...
        //3 获取唯一标识符 openId
        System.out.println(url.toString()+ "==" + tokenUrl + "=" + result);
        JSONObject jsonObject = JSONObject.parseObject(result);
        String session_key = jsonObject.getString("session_key");
        String openid = jsonObject.getString("openid");
