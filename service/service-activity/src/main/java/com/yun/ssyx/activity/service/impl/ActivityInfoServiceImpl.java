package com.yun.ssyx.activity.service.impl;
...
import com.yun.ssyx.activity.mapper.ActivityInfoMapper;
import com.yun.ssyx.activity.mapper.ActivityRuleMapper;
import com.yun.ssyx.activity.mapper.ActivitySkuMapper;
import com.yun.ssyx.activity.service.ActivityInfoService;
import com.yun.ssyx.activity.service.CouponInfoService;
import com.yun.ssyx.enums.ActivityType;
import com.yun.ssyx.model.activity.ActivityInfo;
import com.yun.ssyx.model.activity.ActivityRule;
import com.yun.ssyx.model.activity.ActivitySku;
import com.yun.ssyx.model.activity.CouponInfo;
import com.yun.ssyx.model.order.CartInfo;
import com.yun.ssyx.model.product.SkuInfo;
import com.yun.ssyx.vo.activity.ActivityRuleVo;
import com.yun.ssyx.vo.order.CartInfoVo;
import com.yun.ssyx.vo.order.OrderConfirmVo;
...
public class ActivityInfoServiceImpl extends ServiceImpl<ActivityInfoMapper, ActivityInfo> implements ActivityInfoService {
    ...
    //根据关键字镜像模糊匹配
    @Override
    public List<SkuInfo> findSkuInfoByKeyword1(String keyword) {
        // 1 获取商品信息,调用product feign获取商品信息
        List<SkuInfo> skuInfoList = productFeignClient.findSkuInfoByKeyword(keyword);
        // 2 判断商品是否参加了活动,且没有结束,则可设置新活动
        // 2.1 查询activity.info  , activity.sku  表
        List<Long> ids = skuInfoList.stream().map(SkuInfo::getId).collect(Collectors.toList());
        List<SkuInfo> skuInfoListExit = productFeignClient.findSkuInfoList(ids);
        // 排除已参加活动的商品

        List<SkuInfo> resList = new ArrayList<>();
        for (SkuInfo sku : skuInfoList
        ) {
            //没有参加活动添加到集合内
            if (!skuInfoListExit.contains(sku.getId())) {
                resList.add(sku);
            }
        }
        return resList;
